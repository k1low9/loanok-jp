      ...
      <div class="c_r">
            ...
            <?php
                  if(get_field('cars_itemnumber')){
                    if($cars_store__value === '1' ) {
                      ...
                      echo '<p class="fz_14 phone_btn"><a href="tel:'.$GLOBALS['shop_gloval_var']['fukuoka']['tel'].'">電話で問い合わせる</a></p>';
        ...
        <div class="c_r__col">
          ...
          <?php
                    ...
                    if( $cars_afteroption) {
                        echo '<div class="fz_13 after_option__area"><a href="' . esc_url( get_home_url() ) . '/guarantee.html"><em>1年間無料保証付</em><span>詳しくはこちら > </span></a></div>';
                    } elseif($cars_afteroption_original) {
                        echo '<div class="fz_13 after_option__area"><em>自社無料保証付</em></div>';
                    }
                ?>
          ...
          <section class="c_ll">
                  ...
                  if ($price_details['totalprice']): ?>
                    ...
                    <ul class="fz_12 monthly_price__list">
                      <?php
                      // 月々支払金額を表示する
                      foreach ($price_details['prices'] as $key => $value) {
                        // times（回数）が含まれていて、かつ値がNULLでない場合のみ表示
                        if (strpos($key, 'times') !== false && $value !== NULL) {
                          // キーから数字部分を取り出し（例えば"12times"の"12"）
                          $times = str_replace('times', '', $key);
                          // フォーマットして出力
                          echo "<li>".$times . "回月々 " . number_format($value) . "円</li>";
                        }
                        if ($key === $price_details['installment']) {
                          break;
                      }
                      }
                      ?>
          ...
          <dl class="fz_13 flex list_01">
            ...
            <?php
                        ...
                        if($cars_milage__unit === '1') {
                           ...
                           echo '<div><dt>走行距離</dt><dd><span>' . number_format($milage) . 'km</span></dd></div>';
                        } elseif($cars_milage__unit === '2') {
                          $milage = $cars_detail__group['cars_milage'] * 1000;
                           echo '<div><dt>走行距離</dt><dd><span>' . number_format($milage) . 'mile</span></dd></div>';
                        } else {
                          echo '<div><dt>走行距離</dt><dd><span>' . $cars_detail__group['cars_milage'] . $cars_detail__group['cars_milage__unit'] . '</span></dd></div>';
                        }
                    ?>
            <?php endif; ?>
            <?php
