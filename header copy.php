...
<div class="bodywrap">
    ...
    <header class="header">
        ...
        <div class="l_1 header_wrap">
            ...
            <?php if(is_search() ) : ?>
                <?php $form_type = $_GET['post_type']; if($post_type === 'rentacar' ) :?>
                    <?php $order = $_GET['s']; if($order === '1' ) :?>
                    <h1>カーライフ福岡店のレンタカー</h1>
                    <?php elseif($order === '2') : ?>
                    <h1>カーライフ大阪店のレンタカー</h1>
                    <?php elseif($order === '3') : ?>
                    <h1>カーライフ東京店のレンタカー</h1>
                    <?php endif; ?>
                <?php endif; ?>
            <?php elseif (is_post_type_archive( 'rentacar' )) : ?>
                <h1>カーライフ福岡店、大阪店、TOKYO店のレンタカー</h1>
            <?php elseif( is_singular( 'rentacar' )):?>
                <?php
                ...
                ?>
                <h1> <?php the_title(); ?> カーライフ<?php echo $cars_store__name ; ?>の<?php $terms = get_the_terms($post->ID, 'rentacar_cat'); foreach ($terms as $term) : ?><?php echo $term->name; ?><?php endforeach; ?>レンタカー</h1>
            <?php else: ?>
                <h1>カーライフグループ</h1>
            <?php endif; ?>
            <p class="logoimg"><a href="<?php echo home_url('/'); ?>"><img src="<?php bloginfo('template_url'); ?>/assets/img/logo_head.png" alt="カーライフグループ"></a></p>
            <?php if(is_home()): ?>
            <p class="fz_14 header_description">審査無し！自社ローンだから100％購入出来ます。全国対応ぜひカーライフグループにご相談下さい。<br>提携ローン各種・クレジットカード各種取り扱っております。</p>
            <div class="fz_14 header_banner__wrap">
                <div class="cardbland"><span><img src="<?php bloginfo('template_url'); ?>/assets/img/card_brands.png"></span><span><img src="<?php bloginfo('template_url'); ?>/assets/img/card_brands_p.png"></span></div>

            </div>
            <ul class="md_header__banner">
                <li><a href="<?php echo home_url('/'); ?>rentacar/"><img src="<?php bloginfo('template_url'); ?>/assets/img/rent_banner.png" alt="レンタカー" class="rentcar" width="100"></a></li>
                <li><a href="<?php echo home_url('/'); ?>purchaseform.html"><img src="<?php bloginfo('template_url'); ?>/assets/img/head_banner02.jpg" alt="高額査定" width="100"></a></li>
            </ul>
            <?php endif; ?>
        </div>
        ...
        <div class="navpc">
            <nav class="l_1 mx_1440 flex fz_14">
                <ul class="ddmenu">
                    ...
                    <li class="priority_1st bgg"><a href="<?php echo home_url('/'); ?>uc_main_type.html">車種で探す</a></li>
                    ...
                    <li class="priority_3rd"><a href="<?php echo home_url('/'); ?>rentacar/">レンタカー</a>
                        <ul>
                            <li>
                                <a href="<?php echo home_url('/'); ?>rentacars-tokyo.html">レンタカーTOKYO店</a>
                            </li>
                            <li>
                                <a href="<?php echo home_url('/'); ?>rentacars-osaka.html">レンタカー大阪店</a>
                            </li>
                            <li>
                                <a href="<?php echo home_url('/'); ?>rentacars-fukuoka.html">レンタカー福岡店</a>
                            </li>
                        </ul>
                    </li>
