...
<main class="c_single_wrap" style="padding-top:0;">
<div id="jimnys">
<div class="hero">
  <?php
  foreach($heroimage->getHero() as $val){
    ?>
      <div><img src="<?=plugins_url()?>/carlife-web-application/asset/imgs/<?=get_field('jimny_type')?>/hero/<?=$val?>?=<?=date("Y-m-d")?>" alt=""></div>
      <?php
  }
  ?>
    </div>
    <?php
    $path = WP_PLUGIN_DIR.'/carlife-web-application/templates/'.get_field('jimny_type').'.php';
    if(file_exists($path)){
    include($path);
    }
    ?>
...
if ($price_details['totalprice'] && $price_details['installment_valid']) {
?>
    <p class="fz_14 c_single_pliceTtl">自社ローンでの月々支払い額</p>

    <h5 class="fz_14 month">
        <strong class="monthly_price__ttl">
            <?php echo number_format($price_details['prices']->{$price_details['installment']}); ?>
        </strong>円～
    </h5>
